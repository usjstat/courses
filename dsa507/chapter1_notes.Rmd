...
**Hexagonal binning** is a visualization technique used to handle overplotting in scatterplots, especially when you have large datasets with two continuous/numeric variables. Rather than plotting each individual point, it aggregates data points into hexagon-shaped cells (or "bins") and colors them based on the number of points that fall into each bin.

\*\* When to Use It \*\* -

-   You have a large number of overlapping points in a scatterplot

-   You want to see the density or distribution of points

-   Your variables are both numeric/continuous
...
The hexbin plot above visualizes the relationship between **Runs** and **Balls Faced** across 50 matches. Each hexagon represents a bin, and its color indicates the number of observations that fall within that bin.
